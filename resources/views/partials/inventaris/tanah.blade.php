...
@section('content')
    ...
    <div class="boja-page-header">
        <div class="boja-page-header-text">
            <h1 class="boja-page-title"><i class="fas fa-map-marked-alt"></i> {{ $judul }}</h1>
            <p class="boja-page-subtitle">Data inventaris tanah milik {{ ucwords(strtolower(setting('sebutan_desa') . ' ' . ($desa['nama_desa'] ?? ''))) }}</p>
        </div>
    </div>
...
@push('scripts')
    <script type="text/javascript">
        document.addEventListener("DOMContentLoaded", function() {
            var _url = `{{ ci_route('internal_api.inventaris-tanah') }}`;
                ...
                success: function(response) {
                    ...
                    if (response.data.length) {
                        response.data.forEach(function(el, key) {
                            var a = el.attributes;
                            rows.push('<tr><td style="text-align:center;font-weight:600;color:#6b7280">' + (key+1) + '</td><td style="font-weight:600">' + a.nama_barang + '</td><td>' + a.kode_barang + '<br>' + a.register + '</td><td style="text-align:right">' + a.luas + '</td><td style="text-align:center">' + a.tahun_pengadaan + '</td><td>' + a.letak + '</td><td>' + a.no_sertifikat + '</td><td>' + a.asal + '</td><td style="text-align:right">' + a.harga_format + '</td></tr>');
                            _total += a.harga;
                        });
                        _tfoot.querySelector('th.total').innerHTML = numeral(_total).format();
                        _tbody.innerHTML = rows.join('');
                    } else { _tfoot.remove(); _tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;padding:24px;color:#6b7280">Belum ada data inventaris tanah</td></tr>'; }
                    setTimeout(function() {
