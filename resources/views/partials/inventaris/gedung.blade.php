...
@section('content')
    ...
    <div class="boja-page-header">
        <div class="boja-page-header-text">
            <h1 class="boja-page-title"><i class="fas fa-building"></i> {{ $judul }}</h1>
            <p class="boja-page-subtitle">Data inventaris gedung dan bangunan {{ ucwords(strtolower(setting('sebutan_desa') . ' ' . ($desa['nama_desa'] ?? ''))) }}</p>
        </div>
    </div>
            ...
            <table id="inventaris" class="display" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th rowspan="2" style="vertical-align:middle">Kondisi Bangunan</th>
                        <th rowspan="2" style="vertical-align:middle">Letak / Lokasi</th>
...
@push('scripts')
    <script type="text/javascript">
        document.addEventListener("DOMContentLoaded", function() {
            var _url = `{{ ci_route('internal_api.inventaris-gedung') }}`;
                ...
                success: function(response) {
                    ...
                    if (response.data.length) {
                        response.data.forEach(function(el, key) {
                            var a = el.attributes;
                            rows.push('<tr><td style="text-align:center;font-weight:600;color:#6b7280">' + (key+1) + '</td><td style="font-weight:600">' + a.nama_barang + '</td><td>' + a.kode_barang + '<br>' + a.register + '</td><td style="text-align:center">' + a.kondisi_bangunan + '</td><td>' + a.letak + '</td><td style="text-align:right">' + a.luas + '</td><td>' + a.tanggal_dokument + '</td><td>' + a.no_dokument + '</td><td>' + a.status_tanah + '</td><td>' + a.asal + '</td><td style="text-align:right">' + a.harga_format + '</td></tr>');
                            _total += a.harga;
                        });
                        _tfoot.querySelector('th.total').innerHTML = numeral(_total).format();
                        _tbody.innerHTML = rows.join('');
                    } else { _tfoot.remove(); _tbody.innerHTML = '<tr><td colspan="11" style="text-align:center;padding:24px;color:#6b7280">Belum ada data inventaris gedung</td></tr>'; }
                    setTimeout(function() {
